...
`all-MiniLM-L6-v2` embeddings as the rest of the suite, so building and searching
the store runs **locally and key-free**. The crew itself is tool-calling, so it
...
The tools are idiomatic CrewAI — `index.as_tools()` returns standard
`crewai.tools.BaseTool`s (semantic, keyword, hybrid, SQL), so any `Agent` can use
them. The shared glue lives one level up in [`../_shared/`](../_shared/): `crew.py`
